...
Auto-restore on launch is SILENT: if manifest shows N>0 rows and live IDB is empty/shrunk, restore newest valid snapshot without prompting. Toast only. See src/lib/auto-restore.ts.
Auto-snapshot MUST NOT overwrite a non-empty snapshot with an empty one on the same day — refuse the write and preserve the good file (src/lib/auto-snapshot.ts).
...
- [Auto-restore is silent](mem://constraints/auto-restore-silent) — silent recovery on launch, no user prompt
