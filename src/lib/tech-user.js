...
let inflight = null;
export async function ensureTechSession(force = false) {
...
const UNLOCK_KEY = (id) => `ym_unlocked_${id}`;
export function markCompanyUnlocked(companyId) {
    if (typeof window === "undefined")
        return;
    sessionStorage.setItem(UNLOCK_KEY(companyId), "1");
}
export function isCompanyUnlocked(companyId) {
    if (typeof window === "undefined")
        return false;
