...
async function checkAndWarn() {
    ...
    if (!q.supported || q.quotaBytes === 0)
        return;
    const level = q.percentUsed >= CRIT_PCT ? "crit" : q.percentUsed >= WARN_PCT ? "warn" : "ok";
    // Ask the browser again for persistence when we cross into warn/crit —
    // some browsers only grant it once usage is meaningful.
    if (level !== "ok" && !q.persisted)
    ...
    const stale = Date.now() - lastToastAt > 30 * 60000;
    if (!escalated && !stale)
        return;
    lastLevel = level;
...
export function startStorageWatcher() {
    ...
    if (typeof window === "undefined")
        return;
    watcherStarted = true;
    setTimeout(() => { void checkAndWarn(); }, 8000);
