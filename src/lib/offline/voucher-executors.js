...
async function nextLocalVoucherNumber(companyId, voucherType, voucherDate) {
    const db = await getOfflineDb();
    const rows = await db.cache_vouchers
        .where("company_id")
        .equals(companyId)
        .filter((v) => v?.is_deleted !== true && v?.voucher_type === voucherType)
        .toArray();
    ...
    try {
        ...
        return nextVoucherNumberFor(rule, rows.map((r) => ({
...
async function runLocalItemVoucherCreate(snap) {
    const db = await getOfflineDb();
    ...
    const skipPostings = snap.voucherType === "sales_order" ||
    ...
    if (!skipPostings) {
        ...
        if (snap.itcClass === "capital_goods") {
            ...
            const byId = new Map(cached.filter(Boolean).map((row) => [row.id, row.name]));
        ...
        const postings = await buildItemVoucherPostings(snap.companyId, snap.voucherType, snap.partyId, snap.totals, {
    ...
    await db.transaction("rw", db.cache_vouchers, db.cache_voucher_items, db.cache_voucher_entries, db.cache_bill_sundries, async () => {
        ...
        if (snap.originalVoucherId) {
            ...
            if (orig && String(orig.party_ledger_id ?? "") !== String(snap.partyId ?? "")) {
                ...
                throw new Error(`Party mismatch: This document is linked to ${sourceLabel} ${orig.voucher_number}, which was for a different party.`);
            }
        }
        ...
        if (itemRows.length > 0)
            await db.cache_voucher_items.bulkPut(itemRows);
        if (entryRows.length > 0)
            await db.cache_voucher_entries.bulkPut(entryRows);
        if (sundryRows.length > 0)
            await db.cache_bill_sundries.bulkPut(sundryRows);
    });
    return { voucherId, voucherNumber };
}
async function runLocalEntryVoucherCreate(snap) {
    const db = await getOfflineDb();
    const voucherId = crypto.randomUUID();
    const voucherNumber = await nextLocalVoucherNumber(snap.companyId, snap.voucherType, snap.voucherDate);
    const stamp = nowIso();
    ...
    await db.transaction("rw", db.cache_vouchers, db.cache_voucher_entries, db.cache_bill_allocations, async () => {
        await db.cache_vouchers.put({
            id: voucherId,
            company_id: snap.companyId,
            voucher_type: snap.voucherType,
        ...
        if (allocRows.length > 0)
            await db.cache_bill_allocations.bulkPut(allocRows);
    });
}
function uniqueIds(ids) {
    return Array.from(new Set(ids.filter((id) => Boolean(id))));
}
async function fetchExistingMasterIds(table, companyId, ids) {
    if (ids.length === 0)
        return new Set();
...
function sanitizeMasterRow(table, row) {
...
async function ensureMasterRefsSynced(table, companyId, ids) {
    ...
    const cached = (await Promise.all(missing.map((id) => cache.get(id))));
    ...
    if (unavailable.length > 0) {
        throw new Error(`One or more ${table} used in this voucher are missing from both cloud and offline cache (${unavailable.length} missing). ` +
    ...
    if (toUpsert.length > 0) {
        ...
        const landedIds = new Set((upserted ?? []).map((r) => r.id));
        ...
        if (landedIds.size === 0) {
            throw new Error(`Could not save ${table === "ledgers" ? "ledger" : "item"} references to the cloud. ` +
...
export function registerVoucherExecutor(key, fn) {
    registry[key] = fn;
}
export function getVoucherExecutor(key) {
    return registry[key];
}
// ---------- Item voucher executor -------------------------------------------
export async function runItemVoucherCreate(snap) {
    const { isLocalOnlyMode } = await import("@/lib/local-only-mode");
    if (isLocalOnlyMode()) {
    ...
    if (!skipPostings) {
        ...
        if (snap.itcClass === "capital_goods") {
            ...
            const { data: itemRecs } = await supabase
                ...
                .select("id, name")
                .in("id", ids);
            const byId = new Map((itemRecs ?? []).map((r) => [r.id, r.name]));
            capitalItems = lines.map(({ l, c }) => ({
                name: (byId.get(l.item_id) || l.description || "Capital Asset").trim(),
                taxable_paise: c.taxable_paise,
                cgst_paise: c.cgst_paise,
        ...
        const postings = await buildItemVoucherPostings(snap.companyId, snap.voucherType, partyId, snap.totals, {
            itcClass: snap.itcClass,
            itcEligible: snap.itcEligible,
            capitalItems,
            sundries: (snap.sundries ?? []).map((s) => ({
    ...
    void logActivity({ company_id: snap.companyId, entity_type: "voucher", entity_id: vid, entity_label: `${snap.voucherType} ${voucherNumber}`, action: "create" });
    return { voucherId: vid, voucherNumber };
}
// ---------- Entry voucher executor ------------------------------------------
export async function runEntryVoucherCreate(snap) {
    const { isLocalOnlyMode } = await import("@/lib/local-only-mode");
    if (isLocalOnlyMode()) {
    ...
    if (allocs.length > 0) {
        const { data: created } = await supabase
            ...
            .eq("voucher_type", snap.voucherType)
            .eq("voucher_number", header.voucher_number)
            .maybeSingle();
        const payId = created?.id;
        if (payId) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            await supabase.from("bill_allocations").insert(allocs.map((a) => ({
                company_id: snap.companyId,
                invoice_voucher_id: a.invoice_voucher_id,
...
registerVoucherExecutor(ITEM_VOUCHER_KEY, (snap) => runItemVoucherCreate(snap).then(() => undefined));
registerVoucherExecutor(ENTRY_VOUCHER_KEY, (snap) => runEntryVoucherCreate(snap));
