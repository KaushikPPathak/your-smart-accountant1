...
function readLastRefresh() {
    try {
        return Number(localStorage.getItem(LAST_REFRESH_KEY) ?? 0) || 0;
    }
    catch {
        return 0;
    }
 ...
 */
export async function refreshSessionIfDue(force = false) {
    if (!isOnline())
 ...
 */
export async function warnIfSessionStale() {
    if (toastedExpiry)
        return;
    const last = readLastRefresh();
    if (!last)
        return; // never refreshed → probably first launch; nothing to warn
...
export function daysSinceLastRefresh() {
    const last = readLastRefresh();
    if (!last)
        return 0;
