...
function rememberWorkMode(mode) {
    try {
        localStorage.setItem(LAST_MODE_KEY, mode);
    }
    catch { /* ignore */ }
}
export function getLastWorkMode() {
    try {
        return localStorage.getItem(LAST_MODE_KEY) || null;
    }
    catch {
        return null;
    }
...
async function tick() {
    ...
    if (!cloudOk || isLocalOnlyMode()) {
        try {
            await materializeLocalOnlyOutbox();
        }
        catch { /* ignore */ }
