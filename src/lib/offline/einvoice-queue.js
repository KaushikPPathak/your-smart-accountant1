...
export function subscribeEinvoiceQueue(fn) {
    listeners.add(fn);
    return () => listeners.delete(fn);
}
export async function enqueueEinvoice(args) {
    const now = Date.now();
    const row = {
        kind: args.kind,
    ...
    const id = (await offlineDb.einvoice_queue.add(row));
    notify();
    return id;
}
export async function listEinvoiceQueue(companyId) {
    const rows = (await offlineDb.einvoice_queue.toArray());
    const filtered = companyId ? rows.filter((r) => r.company_id === companyId) : rows;
    return filtered.sort((a, b) => a.created_at - b.created_at);
}
export async function einvoiceQueueSize(companyId) {
    return (await listEinvoiceQueue(companyId)).length;
}
export async function discardEinvoiceQueue(id) {
    await offlineDb.einvoice_queue.delete(id);
    notify();
}
/** Mark a row as pending again so drainEinvoiceQueue picks it up. */
export async function retryEinvoiceQueue(id) {
    const row = (await offlineDb.einvoice_queue.get(id));
    if (!row)
        return;
 ...
 */
export async function drainEinvoiceQueue() {
    if (typeof navigator !== "undefined" && !navigator.onLine) {
