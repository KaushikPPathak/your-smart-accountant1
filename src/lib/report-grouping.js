import { GROUPS_BY_SECTION, defaultGroupCodeForType, } from "@/lib/account-groups";
import { formatINR } from "@/lib/money";
/** Resolve a ledger's effective group code (column or fallback from type). */
export function ledgerGroupCode(b) {
    if (b.group_code)
        return b.group_code;
    return defaultGroupCodeForType(b.type) ?? "CURRENT_ASSETS";
}
/**
 ...
 */
export function groupBalances(balances, section, signFor, innerFor) {
 ...
 */
export function groupedTRows(buckets, onLedgerClick, labelFor) {
    const rows = [];
    let total = 0;
    for (const b of buckets) {
        ...
        for (const r of b.rows) {
            if (r.inner && r.inner.length > 0) {
                ...
                rows.push({
                    label: <span className="pl-3 font-medium">{r.name}</span>,
                    amount: "",
                ...
                for (const inn of r.inner) {
                    rows.push({
                        label: <span className="pl-8 text-muted-foreground">{inn.label}</span>,
                ...
                rows.push({
                    label: <span className="pl-3">{r.name}</span>,
...
export function groupedExportRows(buckets, prefix = "") {
    const out = [];
    for (const b of buckets) {
