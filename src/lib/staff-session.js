...
export function markUnlocked(staff) {
    if (typeof window === "undefined")
        return;
    ...
    if (staff.username)
        localStorage.setItem(LAST_USERNAME_KEY, staff.username.trim().toLowerCase());
}
...
export function lockWorkspace() {
    ...
    for (let i = sessionStorage.length - 1; i >= 0; i--) {
        ...
        if (k && k.startsWith("ym_unlocked_"))
...
export function getActiveStaff() {
    ...
    return {
        ...
        role: localStorage.getItem(STAFF_ROLE_KEY) ?? "staff",
    };
}
