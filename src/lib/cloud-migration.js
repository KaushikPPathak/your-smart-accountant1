...
export async function isCloudMigrationDone() {
    const v = await getMeta(MIGRATION_FLAG);
    return Boolean(v?.at);
}
/**
 ...
 * devices later.
 */
export async function runOneTimeCloudMigrationDown() {
    const result = {
        alreadyDone: false,
        pulledCompanies: 0,
    ...
    const companyIds = Array.from(new Set((memberships ?? []).map((r) => r.company_id)));
    ...
    for (const companyId of companyIds) {
        try {
            ...
            if (snap && Object.keys(snap.errors).length === 0) {
                ...
                result.errors.push(`Pull failed for company ${companyId}: ${snap ? Object.values(snap.errors).join("; ") : "no result"}`);
            }
        }
        catch (e) {
            result.errors.push(`Pull threw for company ${companyId}: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
 ...
 */
export function scheduleCloudMigrationDown() {
                ...
                try {
                    ...
                    if (res.pulledCompanies > 0 || res.releasedMemberships > 0) {
                        toast.success("Your data is now stored on this device only", {
                            description: res.removedCompanies > 0
